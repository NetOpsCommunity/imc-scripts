...
sub cleanupConfiguration
{
	...
	pos($cleanConfig) = 0;
	$start = 0;
	if ( $cleanConfig =~ /(; [\S]+ Configuration)/gc)
	{
		$start = pos($cleanConfig) - length($1) ;
		$end   = index($cleanConfig, "Press RETURN when", $start);

		$cleanConfig = substr($cleanConfig, $start, $end-$start-1);
	}

	$cleanConfig =~ s/(\s*\S+)\s+\(/$1\(/g;
...
sub cleanupVersion
{
	...
	# The model information is printed with the clock. 
	# We're about to strip the clock.
	if ($cleandata =~ /ProCurve Switch ([0-9A-Z]+)/i)
	{
		$model = $1;
	}
	pos($cleandata) = 0;

	if($cleandata =~ /(System +Contact)/gc){
		$start = pos($cleandata) - length($1);
		$end = index($cleandata, "Press RETURN when", $start);

		$cleandata = substr($cleandata, $start, $end - $start - 1);
	}
	
	$cleandata = removeMores($cleandata);

	return "Driver Added Model: $model\n" . $cleandata;
}
