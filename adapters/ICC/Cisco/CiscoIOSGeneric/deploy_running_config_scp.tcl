...
#**************************************************************************
# Identification:deploy_running_config_scp.tcl
# Purpose:       deploy running config to device by scp
#**************************************************************************
...
if { $keyfile != "" } {
   spawn pscp -scp -i \"$keyfile\" $FileName $username@$deviceip:running-config
} else {
   spawn pscp -scp $FileName $username@$deviceip:running-config
}
...
while { $loop == "true" } {
	expect {
		-re $password_prompt {				    
			...
			set ERROR_RESULT  true
			set ERROR_MESSAGE "Internal error."
			set loop false
