...
#**************************************************************************
	...
	if {$pos_ != -1} {
		set my_cmd "archive download-sw /overwrite tftp://@$TFTPServer/$TFTPFile"
    send "$my_cmd\r"
    set loop true
    while {$loop == "true"} {
    	expect {
    		-re "Error" {
    			...
    			expect $enable_prompt
    			set ERROR_RESULT true
					set ERROR_MESSAGE "The directory already exists."
					return
    		} -re "ERROR" {
    			set loop false
    			expect $enable_prompt
	...
	if {$transfer_protocol == "ftp"} {
		if {$FTPUser == "" || $FTPUser == "\x24FTPUser"} {
			...
			send "copy ftp://$FTPUser:"
			send "$FTPPassword"
			send "@$TFTPServer/$TFTPFile $slot\r"
		}
		set loop true
		while {$loop == "true"} {
			expect {
