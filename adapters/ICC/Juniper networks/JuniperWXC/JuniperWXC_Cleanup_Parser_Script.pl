...
sub removeMores
{
	...
	# The More prompt includes a non-printing pause character after the closing paren
	$rawdata = stripCarriageReturns($rawdata);
	$rawdata =~ s/Press any key to continue \(Q to quit\)\s*//g;
	return $rawdata;
}
...
sub cleanupConfiguration
{
	...
	# Trim the header [includes several commands and prompts] by looking for the first line of the config file
	$start = index( $config, "# Config" );
	if ($start > -1) {
